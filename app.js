...
const demo=[
 ...
 {code:'Disinfect-20260718-001',name:'Peracetic Acid Disinfectant',category:'น้ำยา Disinfectant',unit:'แกลลอน',received:'2026-07-18',expiry:'2026-10-15',qty:12,receivedBy:'สมชาย ใจดี'},
 {code:'Filter-20260612-001',name:'Dialyzer High Flux',category:'ตัวกรองเลือด',unit:'ชิ้น',received:'2026-06-12',expiry:'2026-07-22',qty:3,receivedBy:'สมหญิง รักงาน'},
 {code:'Line-20260422-001',name:'Bloodline Set',category:'สายส่งเลือด',unit:'ชุด',received:'2026-04-22',expiry:'2026-07-10',qty:5,receivedBy:'สมหญิง รักงาน'},
...
let loadingCount=0;
function showLoading(text='กำลังดำเนินการ...'){const el=$('#loadingText');if(el)el.textContent=text;loadingCount++;$('#loadingOverlay').hidden=false}
function hideLoading(){loadingCount=Math.max(0,loadingCount-1);if(loadingCount===0)$('#loadingOverlay').hidden=true}
...
function populateCatalog(){$('#categoryList').innerHTML=categories.map(c=>`<option value="${c}"></option>`).join('')}
function updateItemList(){const cat=$('#inCategorySelect').value.trim(),valid=categories.includes(cat),list=valid?catalog.filter(i=>i.category===cat):[];$('#itemList').innerHTML=list.map(i=>`<option value="${i.name}"></option>`).join('');$('#inItem').disabled=!valid;$('#inItem').placeholder=valid?'พิมพ์ค้นหาหรือเลือกรายการ':'เลือกประเภทก่อน';if(!valid||$('#inItem').value.trim()&&!list.some(i=>i.name===$('#inItem').value.trim())){$('#inItem').value='';setInInfo()}}
function genItemCode(categoryThai,receivedDate){const prefix=categoryCode[categoryThai]||'Item',ymd=(receivedDate||today()).replace(/-/g,''),lead=`${prefix}-${ymd}-`,used=items.filter(i=>i.code&&i.code.startsWith(lead)).map(i=>Number(i.code.slice(lead.length))||0),seq=(used.length?Math.max(...used):0)+1;return `${lead}${String(seq).padStart(3,'0')}`}
